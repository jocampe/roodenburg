...
import { newsPosts, Locale } from "./site-data";
...
export function NewsMediaPage({ locale, initialSection = "news" }: { locale: Locale; initialSection?: MediaSection }) {
    ...
    <main className="content-page" id="main-content">
      ...
      {activeSection === "news" && <section className="content-body">
        ...
        <div className="news-archive-grid">
          {newsPosts.filter((item) => item.kind === "news").map((item, index) => (
      ...
      {activeSection === "reports" && <section className="content-body">
        ...
        <div className="report-grid">
          {newsPosts.filter((item) => item.kind === "report").map((item) => <article key={item.slug}><span>{item.category[locale]} · {item.date[locale]}</span><h3>{item.title[locale]}</h3><p>{item.intro[locale]}</p><a href={`${base}/news/${item.slug}`}>{isNl ? "Bekijk verslag" : "View report"} <span>→</span></a></article>)}
        </div>
      </section>}
      ...
      <section className="sample-content-note"><strong>{isNl ? "Conceptinhoud" : "Sample content"}</strong><p>{isNl ? "Berichten, data en verslagen zijn voorbeelden. Later worden deze beheerd vanuit het CMS." : "Posts, dates and reports are examples. They will later be managed through the CMS."}</p></section>
