...
import { Locale, NewsPost, newsPosts as fallbackNewsPosts } from "./site-data";
import { Breadcrumbs, SiteHeader } from "./site-header";

export function ArticlePage({ locale, post, posts = fallbackNewsPosts }: { locale: Locale; post: NewsPost; posts?: readonly NewsPost[] }) {
  const isNl = locale === "nl";
  const base = `/${locale}`;
  const related = posts.filter((item) => item.slug !== post.slug && item.kind === post.kind).slice(0, 3);
  ...
  return <main className="article-page" id="main-content">
    ...
    <section className="article-layout">
      <article className="article-content">
        <figure><img src={post.image} alt={post.imageAlt[locale]} /><figcaption>{isNl ? "Beeld bij dit bericht" : "Article image"}</figcaption></figure>
    ...
    <section className="sample-content-note"><strong>{isNl ? "CMS-koppeling actief" : "CMS connection active"}</strong><p>{isNl ? "Gepubliceerde CMS-inhoud wordt direct gebruikt. Zolang het CMS nog leeg is, blijft de goedgekeurde voorbeeldinhoud beschikbaar." : "Published CMS content is used directly. The approved sample content remains available while the CMS is still empty."}</p></section>
