...
export function ClubPage({ locale, initialSection = "about", initialTopic = "general" }: { locale: Locale; initialSection?: ClubSection; initialTopic?: string }) {
  ...
  useEffect(() => {
    const requested = new URLSearchParams(window.location.search).get("section") as ClubSection | null;
  ...
  return <main className="content-page" id="main-content">
    ...
    {activeSection === "contact" && <section className="content-body contact-section">
      ...
      <div className="contact-layout">
        ...
        <form className="contact-form" onSubmit={submitContact} aria-busy={formState === "submitting"}>
          <div className="contact-form__heading"><span>{isNl ? "Stuur een bericht" : "Send a message"}</span><p>{isNl ? "Kies een onderwerp, dan komt je vraag bij de juiste clubrol terecht." : "Choose a topic so your question reaches the right club role."}</p></div>
          ...
          <div className="contact-form__row"><label><span>{isNl ? "Naam" : "Name"}</span><input name="name" autoComplete="name" minLength={2} maxLength={120} required /></label><label><span>{isNl ? "E-mailadres" : "Email address"}</span><input name="email" type="email" autoComplete="email" maxLength={254} required /></label></div>
          <label><span>{isNl ? "Telefoon (optioneel)" : "Phone (optional)"}</span><input name="phone" type="tel" autoComplete="tel" maxLength={40} /></label>
          <label><span>{isNl ? "Bericht" : "Message"}</span><textarea name="message" rows={6} minLength={10} maxLength={5000} required /></label>
          <label className="contact-consent"><input name="consent" type="checkbox" required /><span>{isNl ? "Ik geef toestemming om mijn gegevens te gebruiken voor het beantwoorden van deze vraag." : "I consent to my details being used to answer this question."}</span></label>
          ...
          <button type="submit" disabled={formState === "submitting"}>{formState === "submitting" ? (isNl ? "Verzenden…" : "Sending…") : (isNl ? "Bericht verzenden" : "Send message")} <span>→</span></button>
          {formState === "success" && <p className="form-status form-status--success" role="status">{isNl ? "Bedankt. Je bericht is veilig ontvangen door de club." : "Thank you. Your message has been received securely by the club."}</p>}
          {formState === "error" && <p className="form-status form-status--error" role="status">{isNl ? "Verzenden is niet gelukt. Controleer je gegevens en probeer het opnieuw." : "Sending failed. Check your details and try again."}</p>}
        </form>
      </div>
    </section>}
