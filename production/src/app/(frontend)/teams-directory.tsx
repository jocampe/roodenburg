...
import { clubTeams as fallbackTeams, ClubTeam, copy, Locale, TeamCategory, teamGroups } from "./site-data";
import { Breadcrumbs, SiteHeader } from "./site-header";

type Filter = "all" | TeamCategory;

export function TeamsDirectory({ locale, initialFilter = "all", teams = fallbackTeams }: { locale: Locale; initialFilter?: Filter; teams?: readonly ClubTeam[] }) {
      ...
      <section className="team-finder" aria-labelledby="team-finder-title">
        ...
        <div className="team-filter" role="group" aria-label={isNl ? "Filter teams" : "Filter teams"}>
          {filters.map((item) => (
            ...
            >
              {item.label}
              <span>{item.id === "all" ? teams.length : teams.filter((team) => team.category === item.id).length}</span>
            </button>
          ))}
        </div>
